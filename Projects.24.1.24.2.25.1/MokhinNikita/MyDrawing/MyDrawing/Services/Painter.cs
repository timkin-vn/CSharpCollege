...
namespace DrawingWindows.Services
{
    public class Painter
    {
        ...
        public void DrawPie(Brush brush, Pen pen, RectangleModel rectangleModel, double start, double sweet)
        {
            var rectangle = _scaler.Scale(rectangleModel);
            if (brush != null)
            {
                _graphics.FillPie(brush, rectangle, (float)start, (float)sweet);
            }
            if (pen != null)
            {
                _graphics.DrawPie(pen, rectangle, (float)start, (float)sweet);
            }
        }
