                ...
                using (var reader = new BinaryReader(fs))
                {
                    while (reader.BaseStream.Position < reader.BaseStream.Length)
                    {
                        ...
                        try
                        {
                            ...
                            newRecord.FullName = reader.ReadString();
                            newRecord.CardNumber = reader.ReadString();
                            newRecord.BonusPoints = reader.ReadInt32();
                            newRecord.CardType = reader.ReadString();
                        }
                        catch
                        {
            ...
            using (var fs = new FileStream(fileName, FileMode.Create, FileAccess.Write))
            {
                using (var writer = new BinaryWriter(fs))
                {
                    foreach (var item in collection.GetAll())
                    {
                        ...
                        writer.Write(item.CardNumber);
                        writer.Write(item.BonusPoints);
                        writer.Write(item.CardType);
                    }
                }
            }
        }
