        ...
        namespace CardFile.Wpf.ViewModels
        {
            public class MainWindowViewModel : ViewModelBase
            {
                ...
                public string WindowTitle => string.IsNullOrEmpty(FileName) ? "Картотека заказов" : $"Картотека заказов: {Path.GetFileName(FileName)}";

public List<string> StatusFilters { get; } = new List<string> { "Все заказы", "Доставленные", "В пути" };

private string _selectedStatusFilter = "Все заказы";
...
if (SelectedStatusFilter == "Доставленные")
    items = items.Where(card => card.ReceivedDate.HasValue);
else if (SelectedStatusFilter == "В пути")
    items = items.Where(card => !card.ReceivedDate.HasValue);
