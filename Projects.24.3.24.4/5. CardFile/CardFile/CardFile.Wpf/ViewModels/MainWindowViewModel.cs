        ...
        namespace CardFile.Wpf.ViewModels
        {
            public class MainWindowViewModel : ViewModelBase
            {
                ...
                public string WindowTitle => string.IsNullOrEmpty(FileName) ? "Картотека видеоигр" : $"Картотека видеоигр: {Path.GetFileName(FileName)}";

public List<string> StatusFilters { get; } = new List<string> { "Все игры", "Пройденные", "Не пройденные" };

private string _selectedStatusFilter = "Все игры";
...
if (SelectedStatusFilter == "Пройденные")
    items = items.Where(card => card.CompletionDate.HasValue);
else if (SelectedStatusFilter == "Не пройденные")
    items = items.Where(card => !card.CompletionDate.HasValue);
