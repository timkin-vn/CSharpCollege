...
namespace Minesweeper.Common
{
    ...
    public static class GameLogic
    {
        ...
        public static bool Apply(GameModel game, int row, int col, CellAction action)
        {
        ...
        public static bool Reveal(GameModel game, int row, int col)
        {
        ...
        public static bool ToggleFlag(GameModel game, int row, int col)
        {
            if (game.IsLost || IsWon(game)) return false;
            if (!InBounds(row, col)) return false;
        ...
        public static bool IsWon(GameModel game)
        {
            ...
            return revealedSafe == n * n - Constants.MineCount;
        }
        ...
        public static int MinesRemaining(GameModel game)
        {
            int flags = 0;
            int n = Constants.Size;
            for (int r = 0; r < n; r++)
        ...
        private static void PlaceMines(GameModel game, int safeRow, int safeCol)
        {
            int n = Constants.Size;
            ...
            int placed = 0;
            while (placed < Constants.MineCount && candidates.Count > 0)
            {
                int pickIndex = Random.Next(candidates.Count);
        ...
        {
            return row >= 0 && row < Constants.Size && col >= 0 && col < Constants.Size;
        }
    }
}
