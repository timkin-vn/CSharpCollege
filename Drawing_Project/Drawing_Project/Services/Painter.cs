...
namespace Drawing.Services
{
    internal class Painter
    {
        ...
        public void DrawTriangular(Brush brush, Pen pen, PointModel[] points)
        {
            var scaledPoints = points.Select(p => Scaler.Scale(p)).ToArray();
            if (brush != null)
            {
                _g.FillPolygon(brush, scaledPoints);
            }
            if (pen != null)
            {
                _g.DrawPolygon(pen, scaledPoints);
            }
        }
