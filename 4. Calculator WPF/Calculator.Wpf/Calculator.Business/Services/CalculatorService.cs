...
namespace Calculator.Business.Services
{
    public class CalculatorService
    {
        ...
        {
            if (double.TryParse(state.InputBuffer, NumberStyles.Any, CultureInfo.InvariantCulture, out double value) && value >= 0)
            {
                state.RegisterX = Math.Sqrt(value);
        ...
        public void Invert(CalculatorState state)
        {
            if (state.RegisterX != 0)
            {
                state.RegisterX = 1 / state.RegisterX;
                state.InputBuffer = state.RegisterX.ToString(CultureInfo.InvariantCulture);
                state.IsClearNeeded = true;
            }
        }
    }
