using CardFile.DataAccess.Dtos;
using CardFile.DataAccess.Infrastructure;
using System;
using System.Collections.Generic;
using System.Linq;

namespace CardFile.DataAccess.DataCollection
{
    public class CardCollection
    {
        private List<CardDto> _cards = new List<CardDto>
        {
            new CardDto
            {
                Id = 1,
                TableNumber = 1,
                CustomerName = "Иван Иванов",
                OrderType = "Еда",
                OrderDate = new DateTime(2025, 6, 4, 12, 0, 0),
                CompletionDate = null,
                Price = 150.00m,
                IsPaid = true,
                WaiterName = "Алексей"
            },
            new CardDto
            {
                Id = 2,
                TableNumber = 2,
                CustomerName = "Анна Петрова",
                OrderType = "Напитки",
                OrderDate = new DateTime(2025, 6, 4, 13, 15, 0),
                CompletionDate = new DateTime(2025, 6, 4, 14, 0, 0),
                Price = 200.00m,
                IsPaid = false,
                WaiterName = "Мария"
            },
            new CardDto
            {
                Id = 3,
                TableNumber = 3,
                CustomerName = "Сергей Сидоров",
                OrderType = "Еда",
                OrderDate = new DateTime(2025, 6, 4, 14, 30, 0),
                CompletionDate = new DateTime(2025, 6, 4, 16, 0, 0),
                Price = 300.00m,
                IsPaid = true,
                WaiterName = "Дмитрий"
            },
            new CardDto
            {
                Id = 4,
                TableNumber = 4,
                CustomerName = "Елена Козлова",
                OrderType = "Напитки",
                OrderDate = new DateTime(2025, 6, 4, 15, 45, 0),
                CompletionDate = null,
                Price = 100.00m,
                IsPaid = true,
                WaiterName = "Ольга"
            }
        };

        internal int CurrentId = 5;

        public CardCollection()
        {
            MapperRegistrator.Register();
        }

        public IEnumerable<CardDto> GetAll()
        {
            return _cards.Select(c => c.Clone()).ToList();
        }

        public int Save(CardDto cardDto)
        {
            if (cardDto.Id == 0)
            {
                var newCard = cardDto.Clone();
                var id = CurrentId++;
                newCard.Id = id;
                _cards.Add(newCard);
                return id;
            }

            var index = _cards.FindIndex(c => c.Id == cardDto.Id);
            if (index >= 0)
            {
                _cards[index] = cardDto.Clone();
                return cardDto.Id;
            }

            return -1;
        }

        public bool Delete(int cardId)
        {
            var index = _cards.FindIndex(c => c.Id == cardId);
            if (index < 0)
            {
                return false;
            }

            _cards.RemoveAt(index);
            return true;
        }

        internal void ReplaceAll(IEnumerable<CardDto> source)
        {
            _cards.Clear();
            _cards.AddRange(source);
            CurrentId = _cards.Any() ? _cards.Max(c => c.Id) + 1 : 1;
        }

        internal void ReplaceAll(IEnumerable<CardDto> source, int currentId)
        {
            _cards.Clear();
            _cards.AddRange(source);
            CurrentId = currentId;
        }
    }
}