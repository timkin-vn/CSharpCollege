...
namespace GraphEditor
{
    public partial class GraphEditorForm : Form
    {
        ...
        private void colorObvodki_Click(object sender, EventArgs e)
        {
            ColorDialog.Color = _service.GetSelectedFillColor();
            if (ColorDialog.ShowDialog() != DialogResult.OK)
            {
        ...
        private void makeBackRect_Click(object sender, EventArgs e)
        {
            OpenDialog.Filter = "Файлы картинок|*.png";
            if (OpenDialog.ShowDialog() != DialogResult.OK)
            {
                return;
            }

            _service.SetSelectedBackgroud(Image.FromFile(OpenDialog.FileName));
            Refresh();
        }
    }
}
