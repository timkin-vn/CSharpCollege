...
namespace FifteenGame.Wpf.ViewModels
{
    public class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        int[] OneColumnRow = { 0, 0 }, TwoColumnRow = { 0, 0 };
        ...
        public void MakeMove(int[] colrow, Action gameFinishedAction)
        {
            if (countbuttonclick == 0)
            {
                ...
                countbuttonclick++;
                _model = _service.CheckMatch(_model.GameId, OneColumnRow, TwoColumnRow);
                FromModel(_model);
        ...
        private void FromModel(GameModel model)
        {
            ...
            for (int row = 0; row < Constants.RowCount; row++)
            {
                for (int column = 0; column < Constants.ColumnCount; column++)
                {
                    ...
                    if ((row == OneColumnRow[0] && column == OneColumnRow[1])
                        || (row == TwoColumnRow[0] && column == TwoColumnRow[1]))
                    {
