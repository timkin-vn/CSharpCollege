...
namespace FrogGame.Business.Services
{
    public class GameService
    {
        ...
        public bool CanReachHome(GameField field)
        {
            var visited = new bool[GameField.RowCount, GameField.ColumnCount];
            return CheckPath(field.FrogRow, field.FrogColumn, visited, field);
        }

        private bool CheckPath(int row, int col, bool[,] visited, GameField field)
        {
            if (row < 0 || row >= GameField.RowCount || col < 0 || col >= GameField.ColumnCount)
                return false;
            ...
            return CheckPath(row + 1, col, visited, field) ||
                   CheckPath(row - 1, col, visited, field) ||
                   CheckPath(row, col + 1, visited, field) ||
                   CheckPath(row, col - 1, visited, field);
        }
    }
}