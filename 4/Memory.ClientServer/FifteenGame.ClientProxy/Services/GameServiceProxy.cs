...
namespace FifteenGame.ClientProxy.Services
{
    public class GameServiceProxy : IGameService
    {
        ...
        {
            var httpContent = JsonContent.Create(new MakeMoveRequest { GameId = gameId, OnebuutonRowCol = OnebuutonRowCol, TwobuutonRowCol = TwobuutonRowCol });
        ...
        private GameModel FromDto(GameReply game)
        {
            ...
            for (int row = 0; row < Constants.RowCount; row++)
            {
                ...
                {
                    model.Field[row, column] = int.Parse(game.Cells[i++]);
                }
            }
