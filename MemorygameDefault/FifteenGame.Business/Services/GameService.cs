...
namespace FifteenGame.Business.Services
{
    ...
    { int countPar = 0;
        ...
        public bool CheckMatch(GameModel model, int[] colrow)
        {
            if (model.Clickbutton == 0)
            {
                model.OneColumnRow = colrow;
                ...
                ||model.OneColumnRow[1] != model.TwoColumnRow[1]))
            {
                ...
                for (int row = 0; row < GameModel.RowCount; row++)
                {
                    for (int column = 0; column < GameModel.ColumnCount; column++)
                    {
                        if ((row == model.OneColumnRow[0] &&
                            column == model.OneColumnRow[1]) || (row == model.TwoColumnRow[0] &&
