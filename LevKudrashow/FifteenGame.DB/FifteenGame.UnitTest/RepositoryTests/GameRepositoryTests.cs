...
namespace FifteenGame.UnitTest.RepositoryTests
{
    ...
    public class GameRepositoryTests
    {
        ...
        public void CreateGameTest()
        {
            ...
            Assert.AreEqual(gameDto.PlayerCount, readGameDto.PlayerCount);
        ...
        public void UpdateGameTest()
        {
            ...
            for (int row = 0; row < Constants.RowCount; row++)
            {
                for (int column = 0; column < Constants.ColumnCount; column++)
                {
                    if (game.Cells[row, column] == Constants.PlayerVal)
                    {
                        freeCellRow = row;
            ...
            game.Cells[changedRow, freeCellColumn] = Constants.PlayerVal;
            game.PlayerCount++;

            gameRepository.Save(game);
            var readGame = gameRepository.GetByGameId(game.Id);

            Assert.AreEqual(game.PlayerCount, readGame.PlayerCount);
