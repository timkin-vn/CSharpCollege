...
namespace GraphEditor.Business.Services
{
    public class PictureService
    {
        ...
        // Новый метод: Переключение формы
        public void ToggleShape()
        {
            if (PictureModel.SelectedRectangle != null)
            {
                PictureModel.SelectedRectangle.IsEllipse = !PictureModel.SelectedRectangle.IsEllipse;
            }
        }

        // Новый метод: Установка прозрачности
        public void SetOpacity(float opacity)
        {
            if (PictureModel.SelectedRectangle != null)
            {
                PictureModel.SelectedRectangle.Opacity = Math.Max(0.0f, Math.Min(1.0f, opacity));
            }
        }
    }
}