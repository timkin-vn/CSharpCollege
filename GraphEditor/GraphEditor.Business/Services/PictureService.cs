...
namespace GraphEditor.Business.Services
{
    public class PictureService
    {
        ...
        private Color _defaultFillColor = Color.Transparent; // Добавляем поле для хранения цвета заливки по умолчанию

        public PictureModel PictureModel => _model;

        public void CreateRectangle(PointModel location)
        {
            var newRectangle = new RectangleModel { Point1 = location, Point2 = location, FillColor = _defaultFillColor }; // Используем цвет заливки по умолчанию
        ...
        public void SetSelectedFillColor(Color color)
        {
            if (_model.SelectedRectangle != null)
            {
                ...
                _defaultFillColor = color; // Обновляем цвет заливки по умолчанию
            }
        }
