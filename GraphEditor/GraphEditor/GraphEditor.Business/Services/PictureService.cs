...
namespace GraphEditor.Business.Services
{
    public class PictureService
    {
        ...
        public void CreateRectangle(PointModel loc, ShapeType shape)
        {
            var newRectangle = new RectangleModel { Left = loc.X, Top = loc.Y, Width = 0, Height = 0, Shape = shape };
            PictureModel.Rectangles.Add(newRectangle);
            PictureModel.SelectedRectangle = newRectangle;
            newRectangle.EditMode = EditMode.Creating;
        }
        ...
        public void UpdateMovingPoint(PointModel loc)
        {
            ...
            switch (selectedRect.EditMode)
            {
                ...
                case EditMode.ResizeBR:
                    ...
                    // do not normalize here so user can drag across anchors (allow inversion while interacting)
                    if (selectedRect.Shape == ShapeType.Triangle)
                    {
                        selectedRect.IsMirrored = false; // always draw triangle pointing up
                    }
                    break;
        ...
        public void MoveForward()
        {
            ...
            int index = PictureModel.Rectangles.IndexOf(selected);
            if (index < PictureModel.Rectangles.Count - 1)
            {
                PictureModel.Rectangles[index] = PictureModel.Rectangles[index + 1];
                PictureModel.Rectangles[index + 1] = selected;
            }
        }
