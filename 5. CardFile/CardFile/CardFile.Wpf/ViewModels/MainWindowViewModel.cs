        ...
        namespace CardFile.Wpf.ViewModels
        {
            public class MainWindowViewModel : ViewModelBase
            {
                ...
                public string WindowTitle => string.IsNullOrEmpty(FileName) ? "Картотека книг" : $"Картотека книг: {Path.GetFileName(FileName)}";
    ...
    set
    {
        ...
        ApplyView();
            ...
            if (!string.IsNullOrWhiteSpace(SearchText))
                items = items.Where(card => (!string.IsNullOrEmpty(card.Title) && card.Title.IndexOf(SearchText, StringComparison.OrdinalIgnoreCase) >= 0) || (!string.IsNullOrEmpty(card.Author) && card.Author.IndexOf(SearchText, StringComparison.OrdinalIgnoreCase) >= 0));
